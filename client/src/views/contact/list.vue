...
export default {
  setup() {
    ...
    function edit(item) {
      router.push(`/contact/edit?id=${item.id}`)
    }
    ...
    function deleteRecord() {
      if (itemToDelete.value) {
        ...
        authStore
          .deleteItem('contact', itemToDelete.value.id)
          .then(() => {
    ...
    onMounted(async () => {
      try {
        const response = await authStore.getList('contact', currentPage.value, pageSize.value)
