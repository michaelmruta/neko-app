...
export default {
  setup() {
    ...
    function edit(item) {
      router.push(`/customer/edit?id=${item.id}`)
    }
    ...
    function deleteRecord() {
      if (itemToDelete.value) {
        ...
        authStore
          .deleteItem('customer', itemToDelete.value.id)
          .then(() => {
    ...
    onMounted(async () => {
      try {
        const response = await authStore.getList('customer', currentPage.value, pageSize.value)
