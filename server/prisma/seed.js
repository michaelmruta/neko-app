...
async function main() {
  ...
  const statuses = ['ACTIVE', 'ACTIVE', 'ACTIVE', 'INACTIVE', 'INACTIVE', 'SUSPENDED', 'PENDING', 'PENDING'];
  
  for (let i = 0; i < 8; i++) {
    const userPassword = await bcrypt.hash(`user${i+1}`, 10);
    const isVerified = ['ACTIVE', 'INACTIVE', 'SUSPENDED'].includes(statuses[i]);
